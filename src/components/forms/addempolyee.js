...
handleDoctorSave = e =>{
        e.preventDefault();
        const { name, email, address, password,gender, ssn, phone,age,usertype} = this.state
    //call api
    axios.post(`http://localhost:5000/addempolyee`,{  name:name, email:email, address:address, password:password, gender: gender,usertype: usertype,
...
resetHandler = (e) => {
  ...
  render() {
    ...
    const { name, email, address, gender, ssn, phone,age,usertype } = this.state
    ...
    return (
      ...
      <div className="home">
        ...
        <form className="user" method="post" enctype="multipart/form-data">
          ...
          <select name="usertype" onChange={this.setStateField('usertype')} className="form-control ">
            <option>choose type </option>
            <option value="3">receptionist</option>
            <option value="5">labspialist</option>
            <option value="6">phamacest</option>
